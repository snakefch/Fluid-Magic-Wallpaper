package com.live.fluidwallpaper.magicapp.services;
...
class GLThread extends Thread {
    ...
    GLThread(GLSurfaceView.Renderer renderer, EGLConfigChooser chooser, EGLContextFactory contextFactory,
        ...
        mHeight = 0;
        mRequestRender = true;
        mRenderMode = GLWallpaperServiceRBG.GLEngine.RENDERMODE_CONTINUOUSLY;
        mRenderer = renderer;
        this.mEGLConfigChooser = chooser;
        this.mEGLContextFactory = contextFactory;
        this.mEGLWindowSurfaceFactory = surfaceFactory;
                ...
                synchronized (sGLThreadManager) {
                    while (true) {
                        ...
                        if ((!mPaused) && mHasSurface && mHaveEgl && (mWidth > 0) && (mHeight > 0)
                                && (mRequestRender || (mRenderMode == GLWallpaperServiceRBG.GLEngine.RENDERMODE_CONTINUOUSLY))) {
    ...
    public void setRenderMode(int renderMode) {
        if (!((GLWallpaperServiceRBG.GLEngine.RENDERMODE_WHEN_DIRTY <= renderMode) && (renderMode <= GLWallpaperServiceRBG.GLEngine.RENDERMODE_CONTINUOUSLY))) {
            throw new IllegalArgumentException("renderMode");
        }
        synchronized (sGLThreadManager) {
            mRenderMode = renderMode;
            if (renderMode == GLWallpaperServiceRBG.GLEngine.RENDERMODE_CONTINUOUSLY) {
                sGLThreadManager.notifyAll();
            }
        }
    }
