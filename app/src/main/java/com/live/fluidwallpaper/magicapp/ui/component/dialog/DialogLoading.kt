...
import com.live.fluidwallpaper.magicapp.databinding.DialogLoadingPresetBinding
...
class DialogLoading(val context: WallpaperLiveViewActivity, val onFinishedLoading: () -> Unit) :
    ...
    private fun showNativeLoading() {
        if (com.live.fluidwallpaper.magicapp.ads.AdsManager.nativeAdLoading != null && !AppPurchase.getInstance().isPurchased) {
            mBinding.frAds.visibleView()

            try {
                ITGAd.getInstance().populateNativeAdView(
                    context,
                    com.live.fluidwallpaper.magicapp.ads.AdsManager.nativeAdLoading,
                    mBinding.frAds,
